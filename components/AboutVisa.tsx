...
const AboutVisa: React.FC = () => {
  ...
  // Fetch and parse the list of qualifications
  ...
  return (
    <div className="container mx-auto px-4 md:px-20 py-12 text-center md:text-left">
      ...
      <ul className="list-disc list-inside mb-6 mx-auto md:mx-0 max-w-xs md:max-w-full">
        {(t('visaAllowsList', { returnObjects: true }) as unknown as string[])?.map((item, index) => (
          <li key={index}>{item}</li>
        ))}
      </ul>
      ...
      <ul className="list-disc list-inside mb-6 mx-auto md:mx-0 max-w-xs md:max-w-full">
        {(t('applyList', { returnObjects: true }) as unknown as string[])?.map((item, index) => (
          <li key={index}>{item}</li>
        ))}
      </ul>
